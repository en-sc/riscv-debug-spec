...
debug_defines:	$(REGISTERS_TEX:.tex=.h)
	echo "/*" > $@.h
	echo " * This file is auto-generated by running 'make debug_defines.h' in" >> $@.h
	echo " * https://github.com/riscv/riscv-debug-spec/ (`git describe --always --dirty --exclude '*'`)" >> $@.h
	echo " */" >> $@.h
	echo >> $@.h
	cat $^ >> $@.h
	./registers.py $@ --cgetters $(patsubst %,xml/%,$(REGISTERS_TEX:.tex=.xml))
